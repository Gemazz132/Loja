...
import { areAssignmentsEqual, getInheritedState, getThemeMode, getThemeModeByState, getModeState, getReference, isModeValue, isReference, isReferenceToken, } from './utils';
/**
...
export function resolveThemeWithPaths(theme, baseTheme, propertiesMap) {
    ...
    Object.keys((_a = baseTheme === null || baseTheme === void 0 ? void 0 : baseTheme.tokens) !== null && _a !== void 0 ? _a : theme.tokens).forEach(function (token) {
        var mode = getThemeMode(baseTheme !== null && baseTheme !== void 0 ? baseTheme : theme, token);
        if (mode) {
...
function resolveModeReferenceTokens(theme, context, baseTheme) {
    var inheritedState = getInheritedState(context);
    if (!inheritedState) {
        return;
    }
    var mode = getThemeModeByState(theme, inheritedState);
    if (!mode) {
        return;
    }
...
export var modeReducer = function (mode, state) {
    return function (tokenResolution, token, theme, baseTheme) {
        var tokenMode = getThemeMode(baseTheme !== null && baseTheme !== void 0 ? baseTheme : theme, token);
